const API_CONFIG = {
  ...
  defaultRepo: 'kanekidies/oxygencs-grp1-eq4'
};
...
function generateSpikesFromRealData(timeSeriesData, avgFailureRate) {
  return timeSeriesData.map(item => {
    ...
    const isDurationSpike = item.avgDuration > timeSeriesData.reduce((s, i) => s + i.avgDuration, 0) / timeSeriesData.length * 1.5;
    const isExecutionSpike = item.runs > timeSeriesData.reduce((s, i) => s + i.runs, 0) / timeSeriesData.length * 1.8;
    ...
    if (isFailureSpike) {
      ...
      anomalyDetail = `Duration +${((item.avgDuration / (timeSeriesData.reduce((s, i) => s + i.avgDuration, 0) / timeSeriesData.length) - 1) * 100).toFixed(0)}% above baseline`;
 ...
 */
function convertApiDataToDashboard(apiData, filters = {}) {
...
/**
 * Fonction principale appelée par le Dashboard - Version avec filtres
 */
export async function fetchDashboardData(filters = {}) {
  try {
    // Récupérer le repo depuis le storage ou utiliser le défaut
    let repo = API_CONFIG.defaultRepo;
    
    try {
      const storage = await chrome.storage.local.get(['currentRepo']);
      if (storage.currentRepo) {
        repo = storage.currentRepo;
        console.log(`🎯 Using stored repo: ${repo}`);
      }
    } catch (e) {
      console.log('⚠️ Could not access storage, using default repo');
    }
    
    console.log(`📡 Loading dashboard data for: ${repo}`);
    console.log(`🔍 Applied filters:`, filters);
    
    // Récupérer les métriques depuis le backend Flask
    const apiData = await fetchMetrics(repo);
    
    // Convertir au format Dashboard avec filtres
    const dashboardData = convertApiDataToDashboard(apiData, filters);
    
    console.log('✅ Dashboard data ready (from real backend):', dashboardData);
