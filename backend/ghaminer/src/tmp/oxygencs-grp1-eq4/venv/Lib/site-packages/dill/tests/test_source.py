...
def test_getsource():
    ...
    assert (
        getsource(Foo) == "class Foo(object):\n  def bar(self, x):\n    return x*x+x\n"
    )
...
def test_importable():
    ...
    assert (
        importable(Foo, source=True)
        == "class Foo(object):\n  def bar(self, x):\n    return x*x+x\n"
    )
    assert importable(Foo.bar, source=True) == "def bar(self, x):\n  return x*x+x\n"
    assert importable(Foo.bar, source=False) == "from %s import bar\n" % __name__
    assert (
        importable(Foo.bar, alias="memo", source=False)
        == "from %s import bar as memo\n" % __name__
    )
    assert (
        importable(Foo, alias="memo", source=False)
        == "from %s import Foo as memo\n" % __name__
    )
    assert (
        importable(squared, alias="memo", source=False)
        == "from %s import squared as memo\n" % __name__
    )
    assert (
        importable(squared, alias="memo", source=True)
        == "memo = squared = lambda x:x**2\n"
    )
    assert (
        importable(add, alias="memo", source=True)
        == "def add(x,y):\n  return x+y\n\nmemo = add\n"
    )
    assert importable(None, alias="memo", source=True) == "memo = None\n"
    assert importable(100, alias="memo", source=True) == "memo = 100\n"
    assert (
        importable(add, builtin=True, source=False) == "from %s import add\n" % __name__
    )
    assert (
        importable(squared, builtin=True, source=False)
        == "from %s import squared\n" % __name__
    )
    assert (
        importable(Foo, builtin=True, source=False) == "from %s import Foo\n" % __name__
    )
    assert (
        importable(Foo.bar, builtin=True, source=False)
        == "from %s import bar\n" % __name__
    )
    assert (
        importable(_foo.bar, builtin=True, source=False)
        == "from %s import bar\n" % __name__
    )
...
def test_numpy():
    try:
        ...
        x = y([1, 2, 3])
        assert (
            importable(x, source=False) == "from numpy import array\narray([1, 2, 3])\n"
        )
        assert importable(y, source=False) == "from %s import array\n" % y.__module__
        assert (
            importable(x, source=True) == "from numpy import array\narray([1, 2, 3])\n"
        )
        ...
        with warnings.catch_warnings():
            ...
            if hasattr(np, "bool"):
                b = "bool_" if np.bool is bool else "bool"
            else:
                b = "bool_"
...
def test_foo():
    assert importable(_foo, source=True).startswith(
        "import dill\nclass Foo(object):\n  def bar(self, x):\n    return x*x+x\ndill.loads("
    )
