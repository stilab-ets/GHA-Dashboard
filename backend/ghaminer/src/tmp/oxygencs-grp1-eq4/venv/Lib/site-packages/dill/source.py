...
def _matchlambda(func, line):
    ...
    if not indentsize(line):
        return False  # FIXME: is this a good check???
...
def getname(obj, force=False, fqn=False):  # XXX: throw(?) to raise error on fail?
    ...
    if fqn:
        return ".".join(_namespace(obj))  # NOTE: returns 'type'
    module = getmodule(obj)
    if not module:  # things like "None" and "1"
        if not force:
            return None  # NOTE: returns 'instance' NOT 'type' #FIXME?
...
def _namespace(obj):
    ...
    try:
        ...
        pass  # str(obj.__class__) made the 'try' unnecessary
    qual = qual.split(".")
    if module in ["builtins", "__builtin__"]:
...
def _importable(
    obj, alias="", source=None, enclosing=False, force=True, builtin=True, lstrip=True
):
    ...
    try:
        if not _isinstance(obj):
            return getimport(
                obj, alias, enclosing=enclosing, verify=(not force), builtin=builtin
            )
        # first 'get the import', then 'get the instance'
        _import = getimport(
            obj, enclosing=enclosing, verify=(not force), builtin=builtin
        )
...
def _closuredimport(func, alias="", builtin=False):
    ...
    for name, obj in list(free_vars.items()):
        ...
        if src.lstrip().startswith("@"):  # we have a decorator
            ...
            candidate = [
                line for line in lines if getname(encl) in line and re.match(pat, line)
            ]
            if not candidate:
    ...
    if not func_vars:
        ...
        if not mod or name is func:  # then it can be handled by getimport
            ...
            candidate = [
                line
                for line in lines
                if getname(name) in line
                and re.match(r".*[\w\s]=\s*" + getname(name) + r"\(", line)
            ]
            if not len(candidate):
                raise TypeError("import could not be found")
            candidate = candidate[-1]
