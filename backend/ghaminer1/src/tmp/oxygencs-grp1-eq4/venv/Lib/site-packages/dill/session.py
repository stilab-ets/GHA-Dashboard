...
__all__ = [
    ...
    "load_module_asdict",
    "dump_session",
    "load_session",  # backward compatibility
]
...
def _stash_modules(main_module):
    ...
    if len(original) < len(main_module.__dict__):
        ...
        if getattr(newmod, "__loader__", None) is None and _is_imported_module(
            main_module
        ):
...
def dump_session(filename=None, main=None, byref=False, **kwds):
    warnings.warn(
        "dump_session() has been renamed dump_module()", PendingDeprecationWarning
    )
...
def _identify_module(file, main=None):
    ...
    try:
        for opcode, arg, pos in genops(file.peek(256)):
            if not found_import:
                if opcode.name in ("GLOBAL", "SHORT_BINUNICODE") and arg.endswith(
                    "_import_module"
                ):
                    found_import = True
            else:
...
def load_module(
    ...
    *Changed in version 0.3.6:* Function ``load_session()`` was renamed to
    ...
    if "main" in kwds:
        warnings.warn(
            "The argument 'main' has been renamed 'module'.", PendingDeprecationWarning
        )
    ...
    try:
        ...
        if pickle_main is not None:
            ...
            error_msg = (
                "can't update{} module{} %r with the saved state of{} module{} %r"
            )
            ...
            if main.__name__ != pickle_main:
                raise ValueError(
                    error_msg.format("", "", "", "") % (main.__name__, pickle_main)
                )
...
def load_session(filename=None, main=None, **kwds):
    warnings.warn(
        "load_session() has been renamed load_module().", PendingDeprecationWarning
    )
...
def load_module_asdict(
    filename: Union[str, os.PathLike] = None, update: bool = False, **kwds
) -> dict:
    """
    ...
    if "module" in kwds:
        raise TypeError(
            "'module' is an invalid keyword argument for load_module_asdict()"
        )
    if hasattr(filename, "read"):
        file = filename
    else:
...
for name in (
    ...
    "_stash_modules",
    "dump_session",
    "load_session",  # backward compatibility functions
):
