...
if NumpyArrayType:  # then has numpy
    ...
    def ndarraysubclassinstance(obj_type):
        if all(
            (c.__module__, c.__name__) != ("numpy", "ndarray") for c in obj_type.__mro__
        ):
            return False
        ...
        if (
            obj_type.__reduce_ex__ is not NumpyArrayType.__reduce_ex__
    ...
    def numpyufunc(obj_type):
        return any(
            (c.__module__, c.__name__) == ("numpy", "ufunc") for c in obj_type.__mro__
        )

    def numpydtype(obj_type):
        if all(
            (c.__module__, c.__name__) != ("numpy", "dtype") for c in obj_type.__mro__
        ):
            return False
...
try:
    ...
    PyBufferedRandomType = (
        None if IS_PYODIDE else get_file_type("r+b", buffering=-1, open=_open)
    )
    PyBufferedReaderType = get_file_type("rb", buffering=-1, open=_open)
    PyBufferedWriterType = get_file_type("wb", buffering=-1, open=_open)
except ImportError:
    PyTextWrapperType = PyBufferedRandomType = PyBufferedReaderType = (
...
def dump(
    obj, file, protocol=None, byref=None, fmode=None, recurse=None, **kwds
):  # , strictio=None):
    """
...
def dumps(
    obj, protocol=None, byref=None, fmode=None, recurse=None, **kwds
):  # , strictio=None):
    """
...
class Pickler(StockPickler):
    ...
    def save(self, obj, save_persistent_id=True):
        ...
        if NumpyArrayType and not (obj_type is type or obj_type in Pickler.dispatch):
            ...
            if ndarraysubclassinstance(obj_type):
                ...
                def save_numpy_array(pickler, obj):
                    ...
                    pickler.save_reduce(
                        _create_array, (f, args, state, npdict), obj=obj
                    )
                    logger.trace(pickler, "# Nu")
                    return
        ...
        if GENERATOR_FAIL and obj_type is GeneratorType:
            msg = (
                "Can't pickle %s: attribute lookup builtins.generator failed"
                % GeneratorType
            )
...
if sys.hexversion >= 0x30B00B0:
    ...
    _incedental_reverse_typemap["GenericAliasIteratorType"] = type(
        iter(GenericAlias(list, (int,)))
    )
    """
...
ALL_CODE_PARAMS = [
    ...
    (
        (3, 11, "a"),
        "co_endlinetable",
        "argcount posonlyargcount kwonlyargcount nlocals stacksize flags code consts names varnames filename name qualname firstlineno linetable endlinetable columntable exceptiontable freevars cellvars",
    ),
    (
        (3, 11),
        "co_exceptiontable",
        "argcount posonlyargcount kwonlyargcount nlocals stacksize flags code consts names varnames filename name qualname firstlineno linetable                          exceptiontable freevars cellvars",
    ),
    (
        (3, 10),
        "co_linetable",
        "argcount posonlyargcount kwonlyargcount nlocals stacksize flags code consts names varnames filename name          firstlineno linetable                                         freevars cellvars",
    ),
    (
        (3, 8),
        "co_posonlyargcount",
        "argcount posonlyargcount kwonlyargcount nlocals stacksize flags code consts names varnames filename name          firstlineno lnotab                                            freevars cellvars",
    ),
    (
        (3, 7),
        "co_kwonlyargcount",
        "argcount                 kwonlyargcount nlocals stacksize flags code consts names varnames filename name          firstlineno lnotab                                            freevars cellvars",
    ),
]
...
def _create_code(*args):
        ...
        if m.case(
            ...
            if CODE_VERSION == (3, 11):
                return CodeType(
                    ...
                    *args[7:14],
                    (
                        args[14].encode() if hasattr(args[14], "encode") else args[14]
                    ),  # linetable
                    (
                        args[15].encode() if hasattr(args[15], "encode") else args[15]
                    ),  # exceptiontable
                    args[16],
                    args[17],
                )
        ...
        # Python 3.10 or 3.8/3.9 (16 members)
            ...
            if CODE_VERSION == (3, 10) or CODE_VERSION == (3, 8):
                return CodeType(
                    ...
                    *args[7:13],
                    (
                        args[13].encode() if hasattr(args[13], "encode") else args[13]
                    ),  # lnotab/linetable
                    args[14],
                    args[15],
                )
        ...
        # Python 3.7 (15 args)
            ...
            if CODE_VERSION == (3, 7):
                return CodeType(
                    ...
                    *args[6:12],
                    (
                        args[12].encode() if hasattr(args[12], "encode") else args[12]
                    ),  # lnotab
                    args[13],
                    args[14],
                )
    ...
    args = (
        (
            fields[k].encode()
            if k in ENCODE_PARAMS and hasattr(fields[k], "encode")
            else fields[k]
        )
...
def _create_filehandle(
    name, mode, position, closed, open, strictio, fmode, fdata
):  # buffering=0
...
def _create_capsule(pointer, name, context, destructor):
    ...
    if attr_found:
        ...
        raise UnpicklingError(
            "%s object exists at %s but a PyCapsule object was expected."
            % (type(capsule), name)
        )
    else:
...
def save_module_dict(pickler, obj):
    if (
        ...
        and obj == pickler._main.__dict__
...
for __dicttype in (dict, OrderedDict):
    __obj = __dicttype()
...
def save_classobj(pickler, obj):  # FIXME: enable pickler._byref
    if not _locate_function(obj, pickler):
        ...
        pickler.save_reduce(
            ClassType, (obj.__name__, obj.__bases__, obj.__dict__), obj=obj
        )
...
def _save_file(pickler, obj, open_):
    ...
    pickler.save_reduce(
        _create_filehandle,
        (obj.name, obj.mode, position, obj.closed, open_, strictio, fmode, fdata),
        obj=obj,
    )
    return
...
if not IS_PYPY:
    ...
    def save_wrapper_descriptor(pickler, obj):
        ...
        pickler.save_reduce(
            _getattr, (obj.__objclass__, obj.__name__, obj.__repr__()), obj=obj
        )
...
def _is_imported_module(module):
    return (
        ...
        or module in sys.modules.values()
    )
    ...
    if False:  # _use_diff:
        if obj.__name__.split(".", 1)[0] != "dill":
            try:
                ...
                pickler.save_reduce(
                    _import_module, (obj.__name__,), obj=obj, state=changed
                )
...
def save_type(pickler, obj, postproc_list=None):
    if obj in _typemap:
        ...
        if not obj._field_defaults:
            _save_with_postproc(
                pickler,
                (_create_namedtuple, (obj.__name__, obj._fields, obj.__module__)),
                obj=obj,
    ...
    # special caes: NoneType, NotImplementedType, EllipsisType, EnumMeta, etc
        ...
        if (
            ...
            _dict, attrs = _get_typedict_type(
                obj, obj.__dict__.copy(), None, postproc_list
...
def save_property(pickler, obj):
    ...
    pickler.save_reduce(type(obj), (obj.fget, obj.fset, obj.fdel, obj.__doc__), obj=obj)
...
if HAS_CTYPES and hasattr(ctypes, "pythonapi"):
    ...
    def save_capsule(pickler, obj):
        ...
        pickler.save_reduce(
            _create_capsule, (pointer, name, context, destructor), obj=obj
        )
